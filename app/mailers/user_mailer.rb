class UserMailer < ApplicationMailer
  ...
  def welcome_email(user)
    @user = user
    @url  = 'http://www.buaaclubs.com/api/users/email/verify/' + Digest::MD5.hexdigest("#{@user.email_verify_code.to_s}")
  ...
  def fp_email(user)
    @user = user
    @url  = 'http://www.buaaclubs.com/api/users/forgetpassword/email/verify/' + Digest::MD5.hexdigest("#{@user.email_verify_code.to_s}")
